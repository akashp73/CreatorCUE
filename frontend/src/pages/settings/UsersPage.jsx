...
const STATUS_CONFIG = {
  ACTIVE:    { label: 'Active',      color: '#10b981', bg: 'rgba(16,185,129,0.15)', icon: Wifi },
  ON_BREAK:  { label: 'On Break',    color: '#f59e0b', bg: 'rgba(245,158,11,0.15)', icon: Coffee },
  IN_MEETING:{ label: 'In Meeting',  color: '#6366f1', bg: 'rgba(99,102,241,0.15)', icon: Video },
  OFFLINE:   { label: 'Offline',     color: '#475569', bg: 'rgba(71,85,105,0.15)',  icon: WifiOff },
}

const ROLE_COLORS = {
  ADMIN:     { color: '#a5b4fc', bg: 'rgba(99,102,241,0.2)' },
  MANAGER:   { color: '#7dd3fc', bg: 'rgba(56,189,248,0.15)' },
  COUNSELLOR:{ color: '#6ee7b7', bg: 'rgba(16,185,129,0.15)' },
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
      <div className="w-full max-w-md rounded-2xl p-6" style={{ background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-bold text-white">{isEdit ? 'Edit Member' : 'Add Team Member'}</h2>
          <button onClick={onClose} style={{ color: '#94a3b8' }}><X size={18} /></button>
        </div>
        <div className="space-y-4">
          <div><label className="text-xs font-semibold mb-1 block" style={{ color: '#94a3b8' }}>Full Name *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="input" /></div>
          {!isEdit && <>
            <div><label className="text-xs font-semibold mb-1 block" style={{ color: '#94a3b8' }}>Email *</label><input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="input" /></div>
            <div><label className="text-xs font-semibold mb-1 block" style={{ color: '#94a3b8' }}>Password *</label><input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} className="input" /></div>
          </>}
          <div><label className="text-xs font-semibold mb-1 block" style={{ color: '#94a3b8' }}>Role</label>
          ...
          {isEdit && <div className="flex items-center gap-3">
            ...
            <label htmlFor="is_active" className="text-sm" style={{ color: '#f1f5f9' }}>Active</label>
          </div>}
  ...
  return (
    <div className="rounded-2xl p-5 space-y-4" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-base relative"
            ...
            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2"
              style={{ background: STATUS_CONFIG[user.status]?.color || '#475569', borderColor: '#0f172a' }} />
          </div>
          <div>
        ...
        <button onClick={() => onEdit(user)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors" style={{ color: '#94a3b8' }}>
          <Edit3 size={14} />
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {breakMins !== null && <span className="text-xs" style={{ color: '#f59e0b' }}>{breakMins}min break</span>}
      </div>

      {/* Today's Stats */}
      <div className="grid grid-cols-3 gap-2 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="text-center">
          <p className="text-lg font-black" style={{ color: '#6366f1' }}>{user.calls_today || 0}</p>
          <p className="text-xs" style={{ color: '#475569' }}>Calls</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-black" style={{ color: '#3b82f6' }}>{user.leads_contacted || 0}</p>
          <p className="text-xs" style={{ color: '#475569' }}>Leads</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-black" style={{ color: '#10b981' }}>{user.conversions || 0}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><Users size={20} style={{ color: '#6366f1' }} /> Team Management</h1>
          <p className="text-sm mt-0.5" style={{ color: '#94a3b8' }}>{users.length} members · {totalActive} active</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Online Now', value: totalActive, color: '#10b981', glow: 'rgba(16,185,129,0.2)' },
          { label: 'On Break', value: onBreak, color: '#f59e0b', glow: 'rgba(245,158,11,0.2)' },
          { label: 'Calls Today', value: totalCalls, color: '#6366f1', glow: 'rgba(99,102,241,0.2)' },
          { label: 'Converted Today', value: totalConversions, color: '#ef4444', glow: 'rgba(239,68,68,0.2)' },
        ].map(s => (
